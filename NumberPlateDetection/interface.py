...
def upload():
    ...
    # print(count())
    lb5.config(text =str(count()), fg='blue')
    ...
    for (x, y, w, h) in number:
        ...
        im_bw = cv.threshold(roi_gray, 128, 255, cv.THRESH_BINARY )[1]
        imgg1 = Image.fromarray(im_bw)
        resized_img1 = imgg1.resize((280, 80), Image.ANTIALIAS)
        new_img1 = ImageTk.PhotoImage(resized_img1)
        img_label2.configure(image=new_img1)
        img_label2.image = new_img1

        # Plate detected picture
        ...
        output = reader.readtext(roi_gray)
        print(output)
        lb4.config(text = str(output[-1][-2]).upper(), fg='green')
...
lb41 = tkinter.Label(text='Plate No. : ', bg="#7DBCE3", font="Bahnschrift 20 bold ")
lb41.place(relx=0.6, rely=0.4 )
lb4 = tkinter.Label(text="XXXXXX", bg="#7DBCE3", font="Bahnschrift 20 bold ")
lb4.place(relx=0.8, rely=0.4 )
lb51 = tkinter.Label(text="Car Passed :", bg="#7DBCE3", font="Bahnschrift 20 bold ")
lb51.place(relx=0.6, rely=0.5 )
lb5 = tkinter.Label(text="0", bg="#7DBCE3", font="Bahnschrift 20 bold ")
lb5.place(relx=0.8, rely=0.5 )
...
# cropped plate
image_path2 = './images/rough2.png'
rough_pic2 = Image.open(image_path2)
